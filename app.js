...
function show() { menu.classList.toggle('block')}

    function firstContry(){
    let x = data[rand(0, 250)];
    headcontent.innerHTML=
    `
   <div id="firstcountry">
        <div id="firstflag"><img src="${x.flag}" alt="country flag"></div>
        <div class="textsecfirst">
            <h2>${x.name}</h2>
            <a href=""><h4>${x.region}</h4></a>
...
let y = 0
function showmore() {
    for (let i = count * y; i < count * (y + 1); i++) {
        let b = data[i]
        let kod = `
        <div class="card">
            <div class="flagsec"><img src="${b.flag}" alt="country flag"></div>
            <div class="textsec">
            <h4>${b.region}</h4>
            <h2>${b.name},  <i>${b.capital}</i></h2>
            <div class="tinybox"><p> Population:${b.area}</p>${b.population}km2</p></div>
            </div>
        </div>
        `;
        maincontent.innerHTML += kod;
    }
    y++;
}
showmore();
...
function nightime() {
    ...
    nav.classList.toggle("darkmode2")
    mode.classList.toggle("moon")
    maincontent.classList.toggle("darkmode3")
    menu.classList.toggle("darkmode2")
}
